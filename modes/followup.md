...
> "No active applications to follow up on. Apply to some roles first with `/JobForge` and come back when they're aging."
...
- Suggest the user run `/JobForge contacto {company}` to find the right person first
...
> - Trying a different contact via `/JobForge contacto`
...
**To:** {email or "No contact found — run `/JobForge contacto` first"}
