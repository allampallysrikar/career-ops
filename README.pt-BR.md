...
<p align="center"><strong>29 scripts de automação · Impulsionado por IA · Criado por Srikar Allampally</strong></p>

<p align="center"></p>

## O que é isso

JobForge transforma qualquer CLI de código com IA em uma central completa de busca de emprego. Em vez de acompanhar candidaturas manualmente em planilha, você tem um pipeline com IA que:
...
> **Importante: isso NÃO é uma ferramenta de disparo em massa.** JobForge é um filtro -- ajuda você a encontrar as poucas vagas que realmente valem seu tempo entre centenas. O sistema recomenda fortemente não se candidatar a nada com nota abaixo de 4.0/5. Seu tempo é valioso, e o do recrutador também. Sempre revise antes de enviar.

JobForge é agentic: Claude Code navega páginas de carreira com Playwright, avalia aderência comparando seu CV com a descrição da vaga (não por simples correspondência de palavras-chave) e adapta seu currículo para cada vaga.
...
Construído por Srikar Allampally para automatizar a busca de emprego do início ao fim.
...
# 6. Comece a usar
# Cole a URL de uma vaga ou rode /JobForge
```
...
JobForge é um único comando slash com múltiplos modos:

```
/JobForge                → Mostrar todos os comandos disponíveis
/JobForge {cole um JD}   → Auto-pipeline completo (avaliar + PDF + tracker)
/JobForge scan           → Escanear portais por novas vagas
/JobForge pdf            → Gerar CV otimizado para ATS
/JobForge batch          → Avaliar múltiplas vagas em lote
/JobForge tracker        → Ver status das candidaturas
/JobForge apply          → Preencher formulários de candidatura com IA
/JobForge pipeline       → Processar URLs pendentes
/JobForge contacto       → Mensagem de outreach no LinkedIn
/JobForge deep           → Pesquisa aprofundada da empresa
/JobForge training       → Avaliar um curso/certificação
/JobForge project        → Avaliar um projeto de portfólio
```

Ou apenas cole uma URL ou descrição de vaga diretamente -- JobForge detecta automaticamente e roda o pipeline completo.
...
- **[cv-santiago](https://github.com/allampallysrikar/cv-santiago)** -- O site de portfólio (github.com/allampallysrikar) com chatbot de IA, dashboard de LLMOps e estudos de caso. Se você precisa de um portfólio para acompanhar sua busca por vagas, faça um fork e adapte para você.

## Sobre o autor

Sou o Srikar — engenheiro de software e criador. Construí o JobForge para automatizar minha própria busca de emprego do início ao fim. Funcionou: usei o sistema para conquistar meu cargo atual.
...
**JobForge é uma ferramenta local e open source — NÃO é um serviço hospedado.** Ao usar este software, você reconhece que:
