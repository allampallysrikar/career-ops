...
<p align="center">
  <a href="https://x.com/allampallysrikar"><img src="docs/hero-banner.jpg" alt="JobForge — 多代理求職系統" width="800"></a>
</p>
...
<p align="center">
  <img src="docs/demo.gif" alt="JobForge 示範" width="800">
</p>

<p align="center"><strong>29 個自動化腳本 · AI 驅動 · Srikar Allampally 打造</strong></p>

<p align="center"></p>

## 這是什麼

JobForge 能將任何 AI 程式碼 CLI 轉化為完整的求職指揮中心。不再需要手動用試算表追蹤應徵紀錄，而是獲得一個 AI 驅動的管道，能夠：
...
> **重要：這不是廣撒網的工具。** JobForge 是一個篩選器 — 它幫助你從數百份職缺中找出真正值得投入的少數機會。本系統強烈建議不要應徵評分低於 4.0/5 的職缺。你的時間很寶貴，招募人員的時間也是。送出前務必仔細審閱。

JobForge 具有代理能力：Claude Code 透過 Playwright 瀏覽求職頁面，藉由推理你的履歷與職缺描述的契合度（而非關鍵字比對）進行評估，並針對每份職缺調整你的履歷。
...
由 Srikar Allampally 打造，旨在將求職流程從頭到尾全面自動化。
...
JobForge 是一個具有多種模式的單一斜線指令：

```
/JobForge                → 顯示所有可用指令
/JobForge {貼上職缺描述}  → 完整自動管道（評估 + PDF + 追蹤）
/JobForge scan           → 掃描平台尋找新職缺
/JobForge pdf            → 生成 ATS 最佳化履歷
/JobForge batch          → 批次評估多份職缺
/JobForge tracker        → 查看應徵狀態
/JobForge apply          → AI 協助填寫應徵表單
/JobForge pipeline       → 處理待辦 URL
/JobForge contacto       → LinkedIn 外寄訊息
/JobForge deep           → 深度公司研究
/JobForge training       → 評估課程/證照
/JobForge project        → 評估作品集專案
```

或者直接貼上職缺 URL 或描述 — JobForge 會自動偵測並執行完整管道。
...
- **[cv-santiago](https://github.com/allampallysrikar/cv-santiago)** — 作者的作品集網站（github.com/allampallysrikar），包含 AI 聊天機器人、LLMOps 儀表板與案例研究。如果你需要一個在求職過程中展示的作品集，可以 fork 它並改造成你自己的。

## 關於作者

我是 Srikar — 軟體工程師與開發者。我打造 JobForge 是為了將自己的求職過程從頭到尾自動化，並成功用它找到了現在這份工作。
...
**JobForge 是一個本地端開源工具 — 並非託管服務。** 使用本軟體即表示你確認：
...
使用 JobForge 找到工作了嗎？[分享你的故事！](https://github.com/allampallysrikar/jobforge/issues/new?template=i-got-hired.yml)
