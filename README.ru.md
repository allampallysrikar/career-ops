...
JobForge превращает любой AI-CLI в полноценный командный центр для поиска работы. Вместо ручного ведения таблиц с откликами вы получаете ИИ-пайплайн, который:
...
> **Важно: это НЕ инструмент для массовой рассылки.** JobForge — это фильтр. Он помогает найти те немногие вакансии, которые стоят вашего времени, из сотен. Система настоятельно рекомендует не откликаться на вакансии с оценкой ниже 4.0/5. Ваше время ценно, как и время рекрутера. Всегда проверяйте перед отправкой.
...
# 6. Начинай
# Вставь URL вакансии или запусти /JobForge
```

## Использование

JobForge — одна slash-команда с множеством режимов:

```
/JobForge                       → Показать все доступные команды
/JobForge {вставь описание JD}  → Полный авто-пайплайн (оценка + PDF + трекер)
/JobForge scan                  → Сканирование порталов
/JobForge pdf                   → Генерация ATS-резюме
/JobForge batch                 → Пакетная оценка вакансий
/JobForge tracker               → Статус откликов
/JobForge apply                 → Заполнение форм отклика с ИИ
/JobForge pipeline              → Обработка очереди URL
/JobForge contacto              → Сообщение в LinkedIn
/JobForge deep                  → Глубокий анализ компании
/JobForge training              → Оценка курса/сертификации
/JobForge project               → Оценка портфолио-проекта
```

Или просто вставь URL вакансии — JobForge автоматически определит его и запустит полный пайплайн.
...
**JobForge — локальный open-source инструмент, НЕ хостинг-сервис.** Используя это ПО, вы подтверждаете:
