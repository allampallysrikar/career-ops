...
<p align="center">
  <a href="https://x.com/allampallysrikar"><img src="docs/hero-banner.jpg" alt="JobForge — マルチエージェント求職システム" width="800"></a>
</p>
...
<p align="center">
  <img src="docs/demo.gif" alt="JobForge デモ" width="800">
</p>
...
JobForgeは、あらゆるAIコーディングCLIを本格的な求職コマンドセンターに変えます。スプレッドシートで応募を手動管理する代わりに、AIによる以下のパイプラインが手に入ります:
...
> **重要: これは「とにかく数を撃つ」ツールではありません。** JobForgeはフィルターです -- 何百もの求人の中から、あなたの時間を割く価値のある数件を見つけ出すためのツールです。本システムは4.0/5未満のスコアの求人への応募を強く非推奨としています。あなたの時間もリクルーターの時間も貴重です。送信前に必ず内容を確認してください。

JobForgeはエージェンティックです: Claude CodeがPlaywrightで求人ページを操作し、（キーワードマッチではなく）あなたのCVと求人票を突き合わせて適合度を推論し、求人ごとにレジュメを最適化します。
...
このシステムは、740件以上の求人を評価し、100件以上のテーラーメイドCVを生成し、Head of Applied AIのポジションを獲得した人物によって作られました。[詳細なケーススタディを読む](https://github.com/allampallysrikar/jobforge-system)。
...
JobForgeは複数のモードを持つ単一のスラッシュコマンドです:

```
/JobForge                → 利用可能なすべてのコマンドを表示
/JobForge {求人票を貼る}  → 完全自動パイプライン（評価 + PDF + トラッカー）
/JobForge scan           → ポータルをスキャンして新しい求人を探す
/JobForge pdf            → ATS最適化CVを生成
/JobForge batch          → 複数オファーをバッチ評価
/JobForge tracker        → 応募ステータスを表示
/JobForge apply          → AIで応募フォームを入力
/JobForge pipeline       → 保留中のURLを処理
/JobForge contacto       → LinkedInアウトリーチメッセージ
/JobForge deep           → 企業の深掘りリサーチ
/JobForge training       → コース/資格を評価
/JobForge project        → ポートフォリオプロジェクトを評価
```

または、単に求人URLや記述を直接貼り付けるだけ -- JobForgeが自動検知してフルパイプラインを実行します。
...
- **[cv-santiago](https://github.com/allampallysrikar/cv-santiago)** -- AIチャットボット、LLMOpsダッシュボード、ケーススタディ付きのポートフォリオサイト（github.com/allampallysrikar）。求職活動と並行して提示するポートフォリオが必要なら、フォークして自分のものにしてください。

## 作者について

Srikarです — ソフトウェアエンジニア、ビルダー。自分自身の求職活動を自動化するためにJobForgeを作りました。効果がありました：このシステムを使って現職を獲得しました。
...
**JobForgeはローカルで動作するオープンソースツールです — ホステッドサービスではありません。** 本ソフトウェアを使用することにより、以下を承諾したものとみなされます:
